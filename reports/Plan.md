...
#### **Валидные данные (в отсутствии документации, исходя из здравого смысла):**
1. `Номер карты`- любой номер, состоящий из 16-ти цифр в формате **** **** **** ****
2. `Месяц` - числовое значение из 2-х цифр (от 01 до 12), не может быть месяцем в прошлом
3. `Год` - числовое значение текущего или следующего года из 2-х цифр (от 22 до 99), не может быть годом в прошлом
4. `Владелец` - фамилия и имя на латинице, состоящие из двух слов через пробел
5. `CVC` - любое число, состоящее из 3-х цифр (от 100 до 999)

#### **Невалидные данные (в отсутствии документации, исходя из здравого смысла):**
1. `Номер карты`- пустое поле, от 0 до 15 цифр, буквы, символы
2. `Месяц` - пустое поле, несуществующий месяц (00, 13), одна цифра, буквы, символы, месяц из прошлого (на момент тестирования от 01 до 09)
3. `Год` - пустое поле, одна цифра, буквы, символы, год из прошлого (00, 21)
4. `Владелец` - пустое поле, ФИ кириллицей, ввод только фамилии/имени латиницей, ввод трех слов через пробел латиницей, 1 буквенный символ, ФИ без пробелов, цифры, символы
5. `CVC` - пустое поле, одна цифра, две цифры, буквы, символы
